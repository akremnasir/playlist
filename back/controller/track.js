...
const viewAll = async (req, res) => {
  const { title, artistName, genere, releseDate, liked, type, albumId } =
    req.query;
  const queryObject = {};
  if (title) {
  ...
  if (!track) {
    return res
      ...
      .json({ success: false, msg: "can not find the required track" });
  }
  res
    .status(StatusCodes.OK)
    .json({ success: true, track, nbhits: track.length });
};
...
const create = async (req, res) => {
  ...
  if (!track) {
    return res
      ...
      .json({ success: false, msg: "can not create track" });
  }
  res
    .status(StatusCodes.CREATED)
    .json({ msg: "created successfully", _id: track._id, track });
};
const update = async (req, res) => {
  ...
  if (!track) {
    return res
      .status(StatusCodes.BAD_REQUEST)
      .json({ success: false, msg: "can not update track" });
  }
...
const deleteOne = async (req, res) => {
  ...
  if (!track) {
    return res
      .status(StatusCodes.BAD_REQUEST)
      .json({ success: false, msg: "can not delete track" });
  }
