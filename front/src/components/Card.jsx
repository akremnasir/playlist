...
import { TrackCardSet, RightContainer, RightButtomContainer, RightUpperContainer, FlexRow, AlignRight, ActiveCard, ActiveCardFlex } from '../styled/styled';
...
const Card = ({reload}) => {
  ...
  const deleteIconClicked = async () => {
    ...
    resp ? () => alert(`${selectedCard.title} deleted successfully`) : alert(`sorry something went wrong`) 
    reload()
  }
  useEffect(() => {
    ...
    <RightContainer>
      ...
      <RightUpperContainer>
        ...
        <ActiveCard>
         ...
         <ActiveCardFlex>
          <FlexRow>
            <p>title:</p><h3>{selectedCard.title}</h3>
          </FlexRow>
          <FlexRow>
            <p>artist name:</p><h3>{selectedCard.artistName}</h3>
          </FlexRow>
          <FlexRow>
            <p>genere:</p><h3>{selectedCard.genere}</h3>
          </FlexRow>
          <FlexRow>
            <p>relese date:</p><h3>{selectedCard.releseDate.slice(0,10)}</h3>
          </FlexRow>
          { selectedCard.type === 'album' && <FlexRow>
            <p>album:</p><h3>{` ${
      ...
      <TrackCardSet>
        {activeItem == 'track' &&trackItems.map((item) => (
          <TrackCard key={item._id} item={item} /> 
        ))}
        {activeItem == 'album' && albumItems.map((item) => (
          <TrackCard key={item._id} item={item} /> 
        ))}
      </TrackCardSet>
    </RightButtomContainer>
