...
import { FormContainer, CloseModal, FormDataContainer, SingleData, FlexRow, InputField, SelectComp, OptinsComp, InputFile } from '../styled/styled'
...
const Form = ({close, reload}) => {
    ...
    const [data, setData] = useState(formType === 'create'? {
      its:'track', 
      title: '', 
      artistName: '',
      genere: '', 
    ...
    const submitFormData = async () => {
      if(formType === 'create'){
        ...
        data.type === 'album' && data.albumId && formData.append("albumId", data.albumId);
        if (data.its === 'track') {
        ...
        <div> 
            <div>
                <CloseModal type="button" onClick = {close}>
                  <svg width={10} padding={10} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                ...
                <FormDataContainer >
                  <FlexRow>
                    <h3>{formType}</h3>
                    { formType === 'create' && (<button onClick={() => data.its === 'track' ? setData({its: 'album'}) : setData({its: 'track'})}>{data.its}</button>)}
                  </FlexRow>
                    
                    <SingleData>
                      <label htmlFor="title">title</label>
                      <InputField type='text' id="title" name="title" value={data.title || ''} onChange={(e) => setData((prev) => ({ ...prev, title: e.target.value }))} />
                    </SingleData>
                    <SingleData >
                      <label htmlFor="artistName">Artist Name</label>
                      <InputField type='text' id="artistName" name="artistName" value={data.artistName} onChange={(e) => setData((prev) => ({ ...prev, artistName: e.target.value }))}/>
                    </SingleData>
                    <SingleData>
                      <label htmlFor="genere">genere</label>
                      <InputField type='text' id="genere" name="genere" value={data.genere} onChange={(e) => setData((prev) => ({ ...prev, genere: e.target.value }))}/>
                    </SingleData>
                    <FlexRow>
                      <label htmlFor="releseDate">Relese Date</label>
                      <InputField type='Date' id="releseDate" name="releseDate" value={data.releseDate} onChange={(e) => setData((prev) => ({ ...prev, releseDate: e.target.value }))}/>
                    </FlexRow>
                    {data.its === 'track' && (<FlexRow>
                      <label htmlFor="type">Type</label>
                      <SelectComp name="type" id="type" value={data.type} onChange={(e) => setData((prev) => ({ ...prev, type: e.target.value }))}>
                        <OptinsComp value="single">Single</OptinsComp>
                        <OptinsComp value="album">Album</OptinsComp>
                      </SelectComp>
                    </FlexRow>)}
                    
                    {data.type === 'album' && (<SingleData>
                      <label htmlFor="albumId">select album</label>
                      <SelectComp name="albumId" id="albumId" value={data.albumId} onChange={(e) => setData((prev) => ({ ...prev, albumId: e.target.value }))}>
                      <OptinsComp value="">Select an album...</OptinsComp>
                      {albumItems[0] ? albumItems.map((item)=>(
